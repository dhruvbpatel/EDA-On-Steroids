...
def main():
    ...
    data = st.file_uploader("Upload Data here (CSV file only)", type=[
                            'csv'], encoding='cp1252')
    ...
    if choice == 'EDA':
        ...
        if data is not None:
            ...
            if st.checkbox("Show Dataset Statistics", False):
                ...
                missing_val_percent = 100 * \
                    (missing_val_sum)/(df.shape[0]*df.shape[1])
                st.write(missing_val_percent, "%")

                st.write("Column Category")
                cat_col = [col for col in df.columns.to_list()
                           if df[col].dtype == 'object']

                cat_col_count = len(cat_col)
                num_col_count = len(df.columns)-cat_col_count
                num_cat_df = pd.DataFrame({"Categorical Column Count": [
                                          cat_col_count], "Numerical Column Count": [num_col_count]}, index=["Counts"])
                st.write(num_cat_df)
...
### individual column summary
    ...
    if choice == 'Data Visualization':
        ...
        if corr_choice == "Show Correlation between among Selected Features":
            # selecting only numerical cols
            num_col = [col for col in df.columns.to_list()
                       if df[col].dtype != 'object']
